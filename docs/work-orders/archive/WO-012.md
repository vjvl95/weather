...
## 완료 기준
- [x] `SettingsPage` — 위치 관리 + GPS 갱신 + 도시 검색 진입
- [x] `CitySearchPage` — 도시 검색 + 선택 → 위치 변경
- [x] `app/city-search.tsx` 라우트 파일 생성 (thin wrapper)
- [x] `src/pages/detail/` 삭제, `app/detail.tsx` 삭제
