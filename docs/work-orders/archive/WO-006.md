...
- **경로**: `@features/weather-fetch/`, `@shared/`
...
// ⚠️ FSD 규칙: 같은 레이어(features) 간 직접 import 금지
// useLocationStore는 widgets/pages 레이어에서 조합
import {
  fetchShortForecast,
  parseCurrentWeather,
  parseHourlyForecast,
...
## 완료 기준
- [x] `src/features/weather-fetch/` 슬라이스 생성 완료
- [x] `useWeatherStore` — 날씨 데이터 CRUD 동작
- [x] `useWeatherFetch` — 기상청 API 호출 + 파싱 + 스토어 저장
- [x] 30분 갱신 간격 로직 동작
